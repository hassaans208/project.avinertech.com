<template>
  <div class="h-screen min-w-screen flex flex-col items-center bg-gradient-to-br from-indigo-900 via-purple-800 to-pink-700 relative overflow-hidden">
    ...
    <div v-if="isCheckingAuth" class="h-screen w-screen flex items-center justify-center">
      <div class="animate-spin h-8 w-8 border-t-2 border-b-2 border-white rounded-full"></div>
    </div>
    
    <!-- Only show login form when auth check is complete -->
    <template v-else>
      <!-- Floating elements -->
      ...
      <div class="w-full max-w-md px-4">
        <div class="text-center mb-8">
          ...
          <p class="mt-2 text-white/80">
            Or
            <Link href="/register" class="font-medium text-teal-400 hover:text-teal-300 transition-colors">
        ...
        <form class="backdrop-blur-xl bg-white/5 rounded-2xl shadow-2xl p-8 border border-white/20 w-full" @submit.prevent="login">
          <div class="space-y-5">
            <div>
              ...
              />
              <p v-if="form.errors.email" class="mt-1 text-sm text-red-300">{{ form.errors.email[0] }}</p>
            </div>
            <div>
              ...
              />
              <p v-if="form.errors.password" class="mt-1 text-sm text-red-300">{{ form.errors.password[0] }}</p>
            </div>
          </div>
...
//     console.log('some',authStore.isAuthenticated);
//     if (authStore.token && !authStore.user) {
//       await authStore.initialize();
//     }
    
//     // If user is authenticated, redirect to setup/dashboard
    
//     if (authStore.isAuthenticated) {
//       // router.visit('/setup/dashboard', { replace: true });
//     }
//   } catch (error) {
...
//   }
//   console.log('some 2',authStore.isAuthenticated);
// });
